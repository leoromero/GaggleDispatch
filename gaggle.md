...
description: >
  ...
  and executes workflow runs in isolated per-issue git worktrees. It is the single authority
...
components:
  ...
  - name: gaggledispatch-repo-syncer
    ...
    description: >
      ...
      `gaggle sync`. Also handles `gaggle repo scaffold`, which runs a workflow to
...
GaggleDispatch is the scheduling and routing layer that sits above all other service repositories in a federated AI coding system. It never writes code or modifies git history — those operations belong to the Claude agents the workflow engine runs inside the target repos. Its responsibilities are reading Linear, deciding where work goes, executing workflows, and writing Linear state back (labels, comments, sub-issues, state transitions).
...
The workflow engine is in-process: GaggleDispatch loads a DAG workflow from the target repo's `.gaggle/workflows/`, runs its nodes (Claude prompts, shell scripts, loops, approval gates) against a git worktree, and records every transition to Postgres. A gate pause is an exact event rather than an inference: the run persists as `paused`, the concurrency slot is freed, a Linear comment posts the gate message, and the orchestrator polls issue comments for a human `approve`/`reject` reply. Durable state lives in two places — Postgres holds run and node state, so a crashed daemon resumes mid-workflow; Linear labels (`gaggle:claimed`, `gaggle:queued`, `gaggle:running`, `gaggle:waiting-human`) hold dispatch state and are read back on startup to reconstruct which targets were in flight.
