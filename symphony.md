...
description: >
  ...
  registry of per-repo symphony.md self-descriptions, fans out to one or more target repositories,
...
components:
  ...
  - name: gaggledispatch-issue-analyzer
    ...
    description: >
      Calls the Anthropic Claude API with the full RegistryContext (every synced repo's symphony.md
  ...
  - name: gaggledispatch-repo-syncer
    ...
    description: >
      ...
      the SHA has changed. Parses each repo's symphony.md after every pull and writes the merged
      ...
      generate a draft symphony.md PR for repos that lack one.
    communicates_with:
  ...
  - name: gaggledispatch-linear-tracker
    component_type: other
    description: >
      GraphQL adapter for the Linear API implementing all 10 operations from the Symphony spec:
      ...
      Auto-creates the four symphony state-machine labels on startup (claimed, queued, running,
      waiting-human).
    communicates_with:
      - component: linear-graphql-api
        method: graphql
  ...
  - name: gaggledispatch-cli
    component_type: cli
    description: >
      Commander-based CLI (invoked as `gaggle` or `symphony`) and the sole operator-facing surface.
